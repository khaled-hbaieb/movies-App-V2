    ...
    const renderItem = ({item, index}) => {
        ...
        <View>
            ...
            <View style={{width:Dimensions.get('window').width /2,height: Dimensions.get('window').width /3,marginTop: 16}}>
                ...
                <Text style={styles.stat1}><FontAwesome name="calendar" size={20} color="white" /> Released     {item.first_air_date}</Text>
...
const renderFavItem = ({item, index}) => {
    ...
    // console.log('from this',typeof item1,'this')
    return(
        <View>
            {/* <TouchableOpacity>
...
const removeFav = async (key) => {
    ...
    useEffect(() => {
        const data = axios.get('https://api.themoviedb.org/3/tv/popular?api_key=d684550d631cad69733c812672083206&language=en-US&page=1')
        .then(result => {
      ...
      const handleSearch = text => {
        ...
        setText(text)
        axios.get(`https://api.themoviedb.org/3/search/tv?query=${text}&api_key=d684550d631cad69733c812672083206`)
        .then(result => {
            console.log(movieSearch,'search heeeeeeere')
    ...
    return (
        <ScrollView >
            ...
            <View style={{...StyleSheet.absoluteFillObject, backgroundColor: '#000'}}>
                ...
                <Text style={styles.topMoviesHeader}>{movieSearch.length === 0 ? 'Top - Tv Shows':`Results for: ${text}`}</Text> 
                <Carousel 
...
export default TvShowsHomeScreen